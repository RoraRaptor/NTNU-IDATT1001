...
public class TextAnalyzer {
    ...
    public TextAnalyzer(String text) {
        ...
        for (int i = 0; i < text.length(); i++) { // Iterate over the string
    ...
    public static void main(String[] args) {
        ...
        while (keepGoing) {
            ...
            char letter = in.next("[a-zA-ZæøåÆØÅ]").charAt(0);
    ...
    public int getLetterAmount(char letter) {
        ...
        if (letter >= 65 && letter <= 90) { // Uppercase letters
            ...
            result = distribution[letter - 97]; // Get the count at the index equivalent of the character
        }
    ...
    public char[] getMostFrequent() {
        ...
        char[] tempList = new char[29]; // Enough space for all combinations of equally frequent letters

        int highestFrequency = 0;

        for (int i = 0; i < tempList.length && i < distribution.length; i++) { // Find the highest frequency of characters
        ...
        for (int i = 0; i < 29; i++) { // Store the characters that have the highest frequency in a temporary list
        ...
        char[] result = new char[resultIndex + 1]; // New list at a more appropriate length
